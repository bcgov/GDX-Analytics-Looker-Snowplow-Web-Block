...
# browser_view_height is not in Sessions
  ...
  dimension: page_display_url_bcs {
    label: "Page Display URL (BCS)"
    # when editing also see clicks.truncated_target_url_nopar_case_insensitive
    description: "Cleaned URL of the page without query string or standard file names like index.html with additional changes for BCS"
    sql: REGEXP_REPLACE(LOWER(${TABLE}.page_display_url), 'default.aspx$','');;
    group_label: "Page"
    drill_fields: [page_display_url,page_views.page_referrer,page_views.page_url]
    link: {
      label: "Visit Page"
      url: "{{ value }}"
      icon_url: "https://looker.com/favicon.ico"
