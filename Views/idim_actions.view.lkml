...
view: idim_actions {
  ...
  derived_table: {
    sql: SELECT name_tracker, ev.app_id, ev.v_tracker AS tracker_version,
      ...
      LEFT JOIN atomic.com_snowplowanalytics_mobile_application_1 AS ma ON ma.root_id = ia.root_id AND ma.root_tstamp = ia.root_tstamp
      LEFT JOIN atomic.com_snowplowanalytics_snowplow_mobile_context_1 AS mc ON mc.root_id = ia.root_id AND mc.root_tstamp = ia.root_tstamp
