...
view: bc_wallet {
  label: "Digital Trust BC Wallet Showcase"
  derived_table: {
    sql:
      WITH list AS (
      SELECT
      ...
      steps AS (
        SELECT
          ...
          progress,
          "timestamp",
          ROW_NUMBER() OVER (PARTITION BY page_urlhost, session_id, path ORDER BY progress DESC) AS step_ranked
        FROM list
      )
      SELECT
        page_urlhost,
